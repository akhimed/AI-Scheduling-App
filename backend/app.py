...
@app.route("/manual_add_shift", methods=["POST"])
def manual_add_shift():
    if "role" not in session or session["role"] != "owner":
        return jsonify({"error": "Unauthorized"}), 403
    ...
    name = data.get('name')
    date = data.get('date')
    availability = data.get('availability')  # like "9 AM - 5 PM"

    if not name or not date or not availability:
        return jsonify({"error": "Missing required fields"}), 400

    try:
        success = database.save_posted_schedule(name, availability, date)
        if success:
            return jsonify({"message": "Shift added successfully"})
        else:
            return jsonify({"error": "Failed to save shift"}), 500
    except Exception as e:
        print(f"❌ Error in manual_add_shift: {e}")
        return jsonify({"error": "Server error"}), 500




from database import get_db_connection  # make sure you have a DB connection function like this

@app.route('/metrics/shifts_per_employee')
def shifts_per_employee():
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute("SELECT name, COUNT(*) AS count FROM posted_schedule GROUP BY name")
    data = [{"name": row[0], "count": row[1]} for row in cursor.fetchall()]
    conn.close()
    return jsonify(data)

@app.route('/metrics/hours_worked')
def hours_worked():
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute("SELECT name, availability FROM posted_schedule")
    data = {}
    for name, availability in cursor.fetchall():
        start, end = availability.split(" - ")
        ...
        data[name] = data.get(name, 0) + hours
    conn.close()
    return jsonify([{"name": name, "hours": hours} for name, hours in data.items()])

@app.route('/metrics/shifts_per_day')
def shifts_per_day():
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute("SELECT date, COUNT(*) FROM posted_schedule GROUP BY date")
    data = [{"date": row[0], "count": row[1]} for row in cursor.fetchall()]
    conn.close()
    return jsonify(data)

@app.route('/metrics/coverage_per_day')
def coverage_per_day():
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute("SELECT date, COUNT(DISTINCT name) FROM posted_schedule GROUP BY date")
    data = [{"date": row[0], "employees": row[1]} for row in cursor.fetchall()]
    conn.close()
    return jsonify(data)

@app.route('/metrics/swap_stats')
def swap_stats():
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute("SELECT status, COUNT(*) FROM swap_requests GROUP BY status")
    counts = {"pending": 0, "accepted": 0, "rejected": 0}
    for row in cursor.fetchall():
        status = row[0] or "sent"
        counts[status] = row[1]
    conn.close()
