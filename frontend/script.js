    ...
    fetch("/view_schedule")
        ...
        .then(data => {
            ...
            const events = data.schedule.map(entry => ({
                ...
                start: new Date(entry.date).toISOString().split("T")[0], // use plain date string directly
                allDay: true,
                editable: isOwner,
...
function setupDashboard() {
    ...
    calendar = new FullCalendar.Calendar(calendarEl, {
        ...
        eventDragStop: function (info) {
            ...
            // Always hide trash after drop
            trash.style.display = "none";

            if (isInTrash) {
                const confirmDelete = confirm(`🗑️ Delete shift for ${info.event.extendedProps.name}?`);
                if (confirmDelete) {
                    info.event.remove(); // Only removes from frontend memory
                    showToast("Shift deleted from draft.", "success");
                }
            }
        }
    });
...
function updateNavbarVisibility() {
    ...
    const analyticsItem = document.querySelector("a[href='dashboard_stats.html']")?.parentElement;
    ...
    if (role) {
        ...
        if (analyticsItem) analyticsItem.style.display = (role === "owner") ? "inline" : "none";
        ...
        if (analyticsItem) analyticsItem.style.display = "none";
    ...
    fetch("/all_availabilities", { credentials: "include" })
        ...
        .then(data => {
            ...
            validShifts.forEach((shift) => {
                ...
                div.classList.add("shift-entry"); // 🌟 Animation class here
                div.innerHTML = `
    ...
    fetch(`/get_swap_requests?username=${username}`)
        ...
        .then(data => {
            ...
            data.forEach(req => {
                ...
                // Parse and format date in UTC to prevent date shifting
                const parsedDate = new Date(Date.parse(req.date));
    ...
    fetch(`/get_swap_requests?from_user=${username}`)
        ...
        .then(data => {
            ...
            data.forEach(req => {
                ...
                // ⏳ Format the swap date
                let readableDate = "Unknown Date";
                if (req.date) {
                    const parsedDate = new Date(Date.parse(req.date));
                    if (!isNaN(parsedDate.getTime())) {
                        readableDate = parsedDate.toLocaleDateString('en-US', {
                            timeZone: 'UTC', // ✅ Added to prevent timezone shift
                            weekday: 'long',
                            year: 'numeric',
                            month: 'short',
                            day: 'numeric'
                ...
                text.textContent = `You requested ${req.to_user} to take: ${req.availability} on ${readableDate}`;
                text.style.flex = "1";
    ...
    fetch(`/get_swap_requests?username=${username}`)
        ...
        .then(requests => {
            ...
            requests.forEach(request => {
                ...
                let readableDate = "Unknown Date";
                if (request.date) {
                    const parsedDate = new Date(Date.parse(request.date));
                    if (!isNaN(parsedDate.getTime())) {
                        readableDate = parsedDate.toLocaleDateString('en-US', {
                            timeZone: 'UTC', // ✅ Added to prevent timezone shift
                            weekday: 'long',
...
async function populateUserDropdown() {
    try {
        const res = await fetch('/all_usernames');
        const data = await res.json();
        const select = document.getElementById('shiftNameSelect');
...
async function submitManualShift() {
    const name = document.getElementById('shiftNameSelect').value;
    const date = document.getElementById('shiftDateInput').value;
    const startTime = document.getElementById('shiftStartTime').value;
    const endTime = document.getElementById('shiftEndTime').value;

    if (!name || !date || !startTime || !endTime) {
        showToast("⛔ Please fill out all fields including date and time.", "warning");
        return;
    }

    if (startTime === "Select start time" || endTime === "Select end time") {
        showToast("⚠️ Please select valid start and end times.", "warning");
        return;
    }
    ...
    // 👇 Instead of saving to backend, just add to calendar
    calendar.addEvent({
    ...
    showToast("✅ Shift added to dashboard!", "success");
}
